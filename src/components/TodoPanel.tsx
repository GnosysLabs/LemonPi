...
export function TodoPanel({ snapshot, hiddenCompletedIds, interrupted = false, paused = false }: { snapshot?: TodoSnapshot; hiddenCompletedIds: Set<number>; interrupted?: boolean; paused?: boolean }) {
  ...
  const planPaused = paused && Boolean(active) && !planInterrupted;
  ...
  return (
    <section className={`todo-panel${collapsed ? " todo-panel--collapsed" : ""}${planInterrupted ? " todo-panel--interrupted" : ""}${planPaused ? " todo-panel--paused" : ""}`} aria-label="Task plan">
      <button className="todo-panel__header" type="button" onClick={() => setCollapsed((value) => !value)} aria-expanded={!collapsed}>
        ...
        <span className="todo-panel__summary">{planInterrupted ? "Stopped · " : planPaused ? "Compacting · " : ""}{completedCount} of {allVisibleTasks.length} complete</span>
          ...
          {tasks.map(({ task, blocked }) => (
            <div className={`todo-task todo-task--${task.status}${blocked ? " todo-task--blocked" : ""}${planInterrupted && task.status === "in_progress" ? " todo-task--interrupted" : ""}${planPaused && task.status === "in_progress" ? " todo-task--paused" : ""}`} key={task.id}>
              ...
              <span className="todo-task__copy">
                ...
                {(task.activeForm || task.description || ((planInterrupted || planPaused) && task.status === "in_progress")) && <small>{planInterrupted && task.status === "in_progress" ? "Stopped before this step completed" : planPaused && task.status === "in_progress" ? "Paused while Pi compacts the conversation context" : task.status === "in_progress" ? task.activeForm ?? task.description : task.description}</small>}
              </span>
