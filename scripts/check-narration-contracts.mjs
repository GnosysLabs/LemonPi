...
assert.match(MAIN_PI_OPERATING_MANUAL, /Opening, resuming, reloading, forking, or navigating to a task is always passive and read-only[\s\S]*never append mission\/todo\/outcome metadata[\s\S]*alter the task's filesystem modification time or sidebar recency[\s\S]*never enqueue a prompt or begin a model turn/);
...
assert.match(injectedPrompt, /lemonpi-main-pi-operating-manual version="7"/);
