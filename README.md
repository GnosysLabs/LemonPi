...
- Makes Main Pi the mission-level Git manager: it handles ordinary low-risk UI changes directly, supervises broader managed-worktree lanes, checkpoints every partial writer, integrates exact owned paths, and reuses unchanged validation evidence.
...
The orchestration replay is a deterministic synthetic fixture, not a wall-clock benchmark of every repository. It exercises policy migration, dirty-tree recovery, worktree ownership, fresh-worker rules, launch preflight, validation deduplication, checkpoint continuation, failure recovery, and visible mission progress. See [Orchestration policy v10](docs/orchestration-policy-v10.md) for the current runtime contract and operator details.
...
LemonPi verifies the required `npm:pi-subagents@0.40.0`, `npm:pi-web-access`, `npm:@juicesharp/rpiv-ask-user-question`, and `npm:@juicesharp/rpiv-todo` packages before launching Pi and asks Pi's native package manager to install any missing or incompatible member. The pinned subagent package receives LemonPi's compiled, exact-match compatibility v1 patch before startup so RPC stop provenance reaches its own status artifact; a changed source fails closed instead of silently reviving the package's hardcoded user-cancellation label. It performs an RPC/lifecycle capability handshake before orchestration. Subagents supply orchestration and machine-readable lifecycle artifacts; web access registers research tools; ask-user-question gives the model a structured clarification tool; and rpiv-todo supplies persistent task snapshots. LemonPi turns the question package's RPC fallback into a native questionnaire and the todo package's public tool-result envelope into a native progress panel. Terminal rendering is never scraped.
...
LemonPi orchestration policy v10 is enforced by the extension runtime rather than model prose alone. Ordinary one-repository UI changes take a mandatory direct fast path with no todo, worker, worktree, reviewer, or broad validation, followed by one exact-path local Git finalization. Broader `lemonpi_dispatch` work uses immutable user-settings provider/model/thinking bindings, runtime-owned outcomes, mechanically expandable writer ownership, deterministic manifests, and managed worktrees. Productivity limits are disabled by default and available only as opt-in per-agent user settings; disabled launches omit all package token, turn, tool, and absolute runtime budgets. Progress supervision uses targeted evidence, diff movement, inactivity, and health checks instead of productive elapsed time. Optional hard limits checkpoint at safe tool boundaries with truthful stop provenance. Continuations start physically from a verified SHA-256 hidden Git checkpoint and use a compact nonrecursive task. `lemonpi_git` integrates the cumulative worker change once while preserving unrelated baseline changes, and `lemonpi_validate` reuses exact revision-plus-diff evidence. Terminal reconciliation is extension-owned and cannot begin before Main Pi and its tools settle. Task navigation and session restoration remain passive and filesystem-read-only: no transcript metadata, timestamp, sidebar recency, or model turn changes without a real user message or a newly observed live worker event.
