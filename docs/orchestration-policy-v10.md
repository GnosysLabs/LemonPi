...
Task navigation is a passive, read-only operation. Startup, reload, resume, fork, and session-tree restoration may replay mission state and reconcile status artifacts in memory, but they never append mission/todo/outcome records, modify the conversation file timestamp or sidebar recency, enqueue a prompt, or authorize a model turn. Delayed persistence timers and queued invisible messages are cleared at the navigation boundary. Session mutation and automatic turns require either a real user message in the open task or a live worker event from a run started in that same session runtime; restored stale state, todo snapshots, timers, and status polling carry neither authority.
